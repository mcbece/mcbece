import { arrayClassify, each, objectMap } from "./index.js"
import { dialog } from "./mdui.js"
...
export function playAudio_v1(srcArr) {
    ...
    $dialog.$content.appendChild(player.audios)
    $dialog.handleUpdate()
}
...
export function playAudio_v2(srcArr) {
    ...
    Promise.all(
        ...
        const $dialog = dialog({
            title: "音频预览",
            content: { toString: () => "" }, // 诈骗一下，直接不填的话后面 .find() 会找不到
            buttons: [
                {
                    text: "分离",
