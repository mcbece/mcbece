import { addValueSetListener, each } from "@/util/index.js"
...
export default class {
    constructor(config) {
        ...
        this.plugin.init(this).then(() => {
            this.config.init()
            addValueSetListener(this.config.$input, () => {
                this.event.emit("app.input", this.config.$input.value)
                if (this.inited) this.change()
            }, { debounce: false }) // TODO 将是否 debounce 加入设置
        }).then(() => {
