...
export default async function(app) {
            ...
            extensionPacks: [
                {
                    ...
                    content: getExtensionData(option, app),
                    __enable: true
                }
            ]
        }
    ...
    .addItem({
        ...
        values: [ [false], [50, "开启，延时 50ms"], [100, "开启，延时 100ms"], [200, "开启，延时 200ms"], [400, "开启，延时 400ms"], [1000, "开启，延时 1000ms (1s)"] ],
        defaultValue: false,
        events: {
            changed: (selected, original) => {
...
class __Option__ extends WebOption {
    ...
    setItem(key, value) {
        confirm({
            ...
            onConfirm: () => this.setItemVal(key, value, (_, __, res) => {
                snackbar("设置成功")
                this.callback(res)
