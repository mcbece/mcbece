...
export const getDefListReplacerConfig = app => ({
    "This:": {
        directReturn(name, listInfo) {
            return listInfo.getItemData(name)
        }
    },
    "Header:": {
        directReturn(name, listInfo) {
            return listInfo.getHeaderData(name)
        }
    },
    "Global:": {
        directReturn(name) {
    ...
    "Color:": {
        directReturn(name) {
            return `<span style="background-color: ${name}; margin: 0 4px; border: 1px inset black">&emsp;</span>`
        },
        indirectReturn: ""
    }
})
