export default [
    {
        "name": "block",
        "minecraft_version": "1.18.10.20",
        "template": {
            "input": {
                "text": "{name} "
            },
            "url": "{normal_page}{info}"
        },
        "__app_list__": true
    },
    {
        "name": "acacia_button",
        "info": "金合欢木按钮"
    },
    {
        "name": "acacia_door",
        "info": "金合欢木门"
    },
    {
        "name": "acacia_fence_gate",
        "info": "金合欢木栅栏门"
    },
    {
        "name": "acacia_pressure_plate",
        "info": "金合欢木压力板"
    },
    {
        "name": "acacia_stairs",
        "info": "金合欢木楼梯"
    },
    {
        "name": "acacia_standing_sign",
        "info": "金合欢木告示牌"
    },
    {
        "name": "acacia_trapdoor",
        "info": "金合欢木活板门"
    },
    {
        "name": "acacia_wall_sign",
        "info": "墙上的金合欢木告示牌"
    },
    {
        "name": "activator_rail",
        "info": "激活铁轨"
    },
    {
        "name": "air",
        "info": "空气"
    },
    {
        "name": "allow",
        "info": "允许方块"
    },
    {
        "name": "amethyst_block",
        "info": "紫水晶块"
    },
    {
        "name": "amethyst_cluster",
        "info": "紫水晶簇"
    },
    {
        "name": "ancient_debris",
        "info": "远古残骸"
    },
    {
        "name": "andesite_stairs",
        "info": "安山岩楼梯"
    },
    {
        "name": "anvil",
        "info": "铁砧"
    },
    {
        "name": "azalea",
        "info": "杜鹃花丛"
    },
    {
        "name": "azalea_leaves",
        "info": "杜鹃树叶"
    },
    {
        "name": "azalea_leaves_flowered",
        "info": "盛开的杜鹃树叶"
    },
    {
        "name": "bamboo",
        "info": "竹子"
    },
    {
        "name": "bamboo_sapling",
        "info": "竹笋"
    },
    {
        "name": "barrel",
        "info": "木桶"
    },
    {
        "name": "barrier",
        "info": "屏障"
    },
    {
        "name": "basalt",
        "info": "玄武岩"
    },
    {
        "name": "beacon",
        "info": "信标"
    },
    {
        "name": "bed",
        "info": "床"
    },
    {
        "name": "bedrock",
        "info": "基岩"
    },
    {
        "name": "bee_nest",
        "info": "蜂巢"
    },
    {
        "name": "beehive",
        "info": "蜂箱"
    },
    {
        "name": "beetroot",
        "info": "甜菜根"
    },
    {
        "name": "bell",
        "info": "钟"
    },
    {
        "name": "big_dripleaf",
        "info": "大型垂滴叶"
    },
    {
        "name": "birch_button",
        "info": "白桦木按钮"
    },
    {
        "name": "birch_door",
        "info": "白桦木门"
    },
    {
        "name": "birch_fence_gate",
        "info": "白桦木栅栏门"
    },
    {
        "name": "birch_pressure_plate",
        "info": "白桦木压力板"
    },
    {
        "name": "birch_stairs",
        "info": "白桦木楼梯"
    },
    {
        "name": "birch_standing_sign",
        "info": "白桦木告示牌"
    },
    {
        "name": "birch_trapdoor",
        "info": "白桦木活板门"
    },
    {
        "name": "birch_wall_sign",
        "info": "墙上的白桦木告示牌"
    },
    {
        "name": "black_candle",
        "info": "黑色蜡烛"
    },
    {
        "name": "black_candle_cake",
        "info": "插上黑色蜡烛的蛋糕"
    },
    {
        "name": "black_glazed_terracotta",
        "info": "黑色带釉陶瓦"
    },
    {
        "name": "blackstone",
        "info": "黑石"
    },
    {
        "name": "blackstone_double_slab",
        "info": "双层黑石台阶"
    },
    {
        "name": "blackstone_slab",
        "info": "黑石台阶"
    },
    {
        "name": "blackstone_stairs",
        "info": "黑石楼梯"
    },
    {
        "name": "blackstone_wall",
        "info": "黑石墙"
    },
    {
        "name": "blast_furnace",
        "info": "高炉"
    },
    {
        "name": "blue_candle",
        "info": "蓝色蜡烛"
    },
    {
        "name": "blue_candle_cake",
        "info": "插上蓝色蜡烛的蛋糕"
    },
    {
        "name": "blue_glazed_terracotta",
        "info": "蓝色带釉陶瓦"
    },
    {
        "name": "blue_ice",
        "info": "蓝冰"
    },
    {
        "name": "bone_block",
        "info": "骨块"
    },
    {
        "name": "bookshelf",
        "info": "书架"
    },
    {
        "name": "border_block",
        "info": "边界"
    },
    {
        "name": "brewing_stand",
        "info": "酿造台"
    },
    {
        "name": "brick_block",
        "info": "砖块"
    },
    {
        "name": "brick_stairs",
        "info": "砖楼梯"
    },
    {
        "name": "brown_candle",
        "info": "棕色蜡烛"
    },
    {
        "name": "brown_candle_cake",
        "info": "插上棕色蜡烛的蛋糕"
    },
    {
        "name": "brown_glazed_terracotta",
        "info": "棕色带釉陶瓦"
    },
    {
        "name": "brown_mushroom",
        "info": "棕色蘑菇"
    },
    {
        "name": "brown_mushroom_block",
        "info": "棕色蘑菇方块"
    },
    {
        "name": "bubble_column",
        "info": "气泡柱"
    },
    {
        "name": "budding_amethyst",
        "info": "紫水晶母岩"
    },
    {
        "name": "cactus",
        "info": "仙人掌"
    },
    {
        "name": "cake",
        "info": "蛋糕"
    },
    {
        "name": "calcite",
        "info": "方解石"
    },
    {
        "name": "campfire",
        "info": "营火"
    },
    {
        "name": "candle",
        "info": "蜡烛"
    },
    {
        "name": "candle_cake",
        "info": "插上蜡烛的蛋糕"
    },
    {
        "name": "carpet",
        "info": "地毯"
    },
    {
        "name": "carrots",
        "info": "胡萝卜"
    },
    {
        "name": "cartography_table",
        "info": "制图台"
    },
    {
        "name": "carved_pumpkin",
        "info": "雕刻过的南瓜"
    },
    {
        "name": "cauldron",
        "info": "炼药锅"
    },
    {
        "name": "cave_vines",
        "info": "洞穴藤蔓"
    },
    {
        "name": "cave_vines_body_with_berries",
        "info": "洞穴藤蔓植株"
    },
    {
        "name": "cave_vines_head_with_berries",
        "info": "洞穴藤蔓植株"
    },
    {
        "name": "chain",
        "info": "锁链"
    },
    {
        "name": "chain_command_block",
        "info": "连锁型命令方块"
    },
    {
        "name": "chest",
        "info": "箱子"
    },
    {
        "name": "chiseled_deepslate",
        "info": "錾制深板岩"
    },
    {
        "name": "chiseled_nether_bricks",
        "info": "錾制下界砖块"
    },
    {
        "name": "chiseled_polished_blackstone",
        "info": "錾制磨制黑石"
    },
    {
        "name": "chorus_flower",
        "info": "紫颂花"
    },
    {
        "name": "chorus_plant",
        "info": "紫颂植株"
    },
    {
        "name": "clay",
        "info": "黏土块"
    },
    {
        "name": "coal_block",
        "info": "煤炭块"
    },
    {
        "name": "coal_ore",
        "info": "煤矿石"
    },
    {
        "name": "cobbled_deepslate",
        "info": "深板岩圆石"
    },
    {
        "name": "cobbled_deepslate_double_slab",
        "info": "双层深板岩圆石台阶"
    },
    {
        "name": "cobbled_deepslate_slab",
        "info": "深板岩圆石台阶"
    },
    {
        "name": "cobbled_deepslate_stairs",
        "info": "深板岩圆石楼梯"
    },
    {
        "name": "cobbled_deepslate_wall",
        "info": "深板岩圆石墙"
    },
    {
        "name": "cobblestone",
        "info": "圆石"
    },
    {
        "name": "cobblestone_wall",
        "info": "圆石墙"
    },
    {
        "name": "cocoa",
        "info": "可可果"
    },
    {
        "name": "command_block",
        "info": "命令方块"
    },
    {
        "name": "composter",
        "info": "堆肥桶"
    },
    {
        "name": "concrete",
        "info": "混凝土"
    },
    {
        "name": "concretepowder",
        "info": "混凝土粉末"
    },
    {
        "name": "conduit",
        "info": "潮涌核心"
    },
    {
        "name": "copper_block",
        "info": "铜块"
    },
    {
        "name": "copper_ore",
        "info": "铜矿石"
    },
    {
        "name": "coral",
        "info": "珊瑚"
    },
    {
        "name": "coral_block",
        "info": "珊瑚块"
    },
    {
        "name": "coral_fan",
        "info": "珊瑚扇"
    },
    {
        "name": "coral_fan_dead",
        "info": "失活的珊瑚扇"
    },
    {
        "name": "coral_fan_hang",
        "info": "墙上的珊瑚扇"
    },
    {
        "name": "coral_fan_hang2",
        "info": "墙上的珊瑚扇"
    },
    {
        "name": "coral_fan_hang3",
        "info": "墙上的珊瑚扇"
    },
    {
        "name": "cracked_deepslate_bricks",
        "info": "裂纹深板岩砖"
    },
    {
        "name": "cracked_deepslate_tiles",
        "info": "裂纹深板岩瓦"
    },
    {
        "name": "cracked_nether_bricks",
        "info": "裂纹下界砖块"
    },
    {
        "name": "cracked_polished_blackstone_bricks",
        "info": "裂纹磨制黑石砖"
    },
    {
        "name": "crafting_table",
        "info": "工作台"
    },
    {
        "name": "crimson_button",
        "info": "绯红木按钮"
    },
    {
        "name": "crimson_door",
        "info": "绯红木门"
    },
    {
        "name": "crimson_double_slab",
        "info": "双层绯红木台阶"
    },
    {
        "name": "crimson_fence",
        "info": "绯红木栅栏"
    },
    {
        "name": "crimson_fence_gate",
        "info": "绯红木栅栏门"
    },
    {
        "name": "crimson_fungus",
        "info": "绯红菌"
    },
    {
        "name": "crimson_hyphae",
        "info": "绯红菌核"
    },
    {
        "name": "crimson_nylium",
        "info": "绯红菌岩"
    },
    {
        "name": "crimson_planks",
        "info": "绯红木板"
    },
    {
        "name": "crimson_pressure_plate",
        "info": "绯红木压力板"
    },
    {
        "name": "crimson_roots",
        "info": "绯红菌索"
    },
    {
        "name": "crimson_slab",
        "info": "绯红木台阶"
    },
    {
        "name": "crimson_stairs",
        "info": "绯红木楼梯"
    },
    {
        "name": "crimson_standing_sign",
        "info": "绯红木告示牌"
    },
    {
        "name": "crimson_stem",
        "info": "绯红菌柄"
    },
    {
        "name": "crimson_trapdoor",
        "info": "绯红木活板门"
    },
    {
        "name": "crimson_wall_sign",
        "info": "墙上的绯红木告示牌"
    },
    {
        "name": "crying_obsidian",
        "info": "哭泣的黑曜石"
    },
    {
        "name": "cut_copper",
        "info": "切制铜块"
    },
    {
        "name": "cut_copper_slab",
        "info": "切制铜台阶"
    },
    {
        "name": "cut_copper_stairs",
        "info": "切制铜楼梯"
    },
    {
        "name": "cyan_candle",
        "info": "青色蜡烛"
    },
    {
        "name": "cyan_candle_cake",
        "info": "插上青色蜡烛的蛋糕"
    },
    {
        "name": "cyan_glazed_terracotta",
        "info": "青色带釉陶瓦"
    },
    {
        "name": "dark_oak_button",
        "info": "深色橡木按钮"
    },
    {
        "name": "dark_oak_door",
        "info": "深色橡木门"
    },
    {
        "name": "dark_oak_fence_gate",
        "info": "深色橡木栅栏门"
    },
    {
        "name": "dark_oak_pressure_plate",
        "info": "深色橡木压力板"
    },
    {
        "name": "dark_oak_stairs",
        "info": "深色橡木楼梯"
    },
    {
        "name": "dark_oak_trapdoor",
        "info": "深色橡木活板门"
    },
    {
        "name": "dark_prismarine_stairs",
        "info": "暗海晶石楼梯"
    },
    {
        "name": "darkoak_standing_sign",
        "info": "深色橡木告示牌"
    },
    {
        "name": "darkoak_wall_sign",
        "info": "墙上的深色橡木告示牌"
    },
    {
        "name": "daylight_detector",
        "info": "阳光探测器"
    },
    {
        "name": "daylight_detector_inverted",
        "info": "反向阳光探测器"
    },
    {
        "name": "deadbush",
        "info": "枯萎的灌木"
    },
    {
        "name": "deepslate",
        "info": "深板岩"
    },
    {
        "name": "deepslate_brick_double_slab",
        "info": "双层深板岩砖台阶"
    },
    {
        "name": "deepslate_brick_slab",
        "info": "深板岩砖台阶"
    },
    {
        "name": "deepslate_brick_stairs",
        "info": "深板岩砖楼梯"
    },
    {
        "name": "deepslate_brick_wall",
        "info": "深板岩砖墙"
    },
    {
        "name": "deepslate_bricks",
        "info": "深板岩砖"
    },
    {
        "name": "deepslate_coal_ore",
        "info": "深层煤矿石"
    },
    {
        "name": "deepslate_copper_ore",
        "info": "深层铜矿石"
    },
    {
        "name": "deepslate_diamond_ore",
        "info": "深层钻石矿石"
    },
    {
        "name": "deepslate_emerald_ore",
        "info": "深层绿宝石矿石"
    },
    {
        "name": "deepslate_gold_ore",
        "info": "深层金矿石"
    },
    {
        "name": "deepslate_iron_ore",
        "info": "深层铁矿石"
    },
    {
        "name": "deepslate_lapis_ore",
        "info": "深层青金石矿石"
    },
    {
        "name": "deepslate_redstone_ore",
        "info": "深层红石矿石"
    },
    {
        "name": "deepslate_tile_double_slab",
        "info": "双层深板岩瓦台阶"
    },
    {
        "name": "deepslate_tile_slab",
        "info": "深板岩瓦台阶"
    },
    {
        "name": "deepslate_tile_stairs",
        "info": "深板岩瓦楼梯"
    },
    {
        "name": "deepslate_tile_wall",
        "info": "深板岩瓦墙"
    },
    {
        "name": "deepslate_tiles",
        "info": "深板岩瓦"
    },
    {
        "name": "deny",
        "info": "拒绝方块"
    },
    {
        "name": "detector_rail",
        "info": "探测铁轨"
    },
    {
        "name": "diamond_block",
        "info": "钻石块"
    },
    {
        "name": "diamond_ore",
        "info": "钻石矿石"
    },
    {
        "name": "diorite_stairs",
        "info": "闪长岩楼梯"
    },
    {
        "name": "dirt",
        "info": "泥土/砂土",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "泥土"
            },
            {
                "name": "1",
                "info": "砂土"
            }
        ]
    },
    {
        "name": "dirt_with_roots",
        "info": "缠根泥土"
    },
    {
        "name": "dispenser",
        "info": "发射器"
    },
    {
        "name": "double_cut_copper_slab",
        "info": "双层切制铜台阶"
    },
    {
        "name": "double_plant",
        "info": "向日葵/丁香/高草丛/大型蕨/玫瑰丛/牡丹",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "向日葵"
            },
            {
                "name": "1",
                "info": "丁香"
            },
            {
                "name": "2",
                "info": "高草丛"
            },
            {
                "name": "3",
                "info": "大型蕨"
            },
            {
                "name": "4",
                "info": "玫瑰丛"
            },
            {
                "name": "5",
                "info": "牡丹"
            }
        ]
    },
    {
        "name": "double_stone_slab",
        "info": "双层平滑石台阶/双层砂岩台阶/双层石化橡木台阶/双层圆石台阶/双层砖台阶/双层石砖台阶/双层石英台阶/双层下界砖台阶",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "双层平滑石台阶"
            },
            {
                "name": "1",
                "info": "双层砂岩台阶"
            },
            {
                "name": "2",
                "info": "双层石化橡木台阶"
            },
            {
                "name": "3",
                "info": "双层圆石台阶"
            },
            {
                "name": "4",
                "info": "双层砖台阶"
            },
            {
                "name": "5",
                "info": "双层石砖台阶"
            },
            {
                "name": "6",
                "info": "双层石英台阶"
            },
            {
                "name": "7",
                "info": "双层下界砖台阶"
            }
        ]
    },
    {
        "name": "double_stone_slab2",
        "info": "双层红砂岩台阶/双层紫珀台阶/双层海晶石台阶/双层暗海晶石台阶/双层海晶石砖台阶/双层苔石台阶/双层平滑砂岩台阶/双层红色下界砖台阶",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "双层红砂岩台阶"
            },
            {
                "name": "1",
                "info": "双层紫珀台阶"
            },
            {
                "name": "2",
                "info": "双层海晶石台阶"
            },
            {
                "name": "3",
                "info": "双层暗海晶石台阶"
            },
            {
                "name": "4",
                "info": "双层海晶石砖台阶"
            },
            {
                "name": "5",
                "info": "双层苔石台阶"
            },
            {
                "name": "6",
                "info": "双层平滑砂岩台阶"
            },
            {
                "name": "7",
                "info": "双层红色下界砖台阶"
            }
        ]
    },
    {
        "name": "double_stone_slab3",
        "info": "双层末地石砖台阶/双层平滑红砂岩台阶/双层磨制安山岩台阶/双层安山岩台阶/双层闪长岩台阶/双层磨制闪长岩台阶/双层花岗岩台阶/双层磨制花岗岩台阶",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "双层末地石砖台阶"
            },
            {
                "name": "1",
                "info": "双层平滑红砂岩台阶"
            },
            {
                "name": "2",
                "info": "双层磨制安山岩台阶"
            },
            {
                "name": "3",
                "info": "双层安山岩台阶"
            },
            {
                "name": "4",
                "info": "双层闪长岩台阶"
            },
            {
                "name": "5",
                "info": "双层磨制闪长岩台阶"
            },
            {
                "name": "6",
                "info": "双层花岗岩台阶"
            },
            {
                "name": "7",
                "info": "双层磨制花岗岩台阶"
            }
        ]
    },
    {
        "name": "double_stone_slab4",
        "info": "双层苔石砖台阶/双层平滑石英台阶/双层石台阶/双层切制砂岩台阶/双层切制红砂岩台阶",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "双层苔石砖台阶"
            },
            {
                "name": "1",
                "info": "双层平滑石英台阶"
            },
            {
                "name": "2",
                "info": "双层石台阶"
            },
            {
                "name": "3",
                "info": "双层切制砂岩台阶"
            },
            {
                "name": "4",
                "info": "双层切制红砂岩台阶"
            }
        ]
    },
    {
        "name": "double_wooden_slab",
        "info": "双层橡木台阶/双层云杉木台阶/双层白桦木台阶/双层丛林木台阶/双层金合欢木台阶/双层深色橡木台阶",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "双层橡木台阶"
            },
            {
                "name": "1",
                "info": "双层云杉木台阶"
            },
            {
                "name": "2",
                "info": "双层白桦木台阶"
            },
            {
                "name": "3",
                "info": "双层丛林木台阶"
            },
            {
                "name": "4",
                "info": "双层金合欢木台阶"
            },
            {
                "name": "5",
                "info": "双层深色橡木台阶"
            }
        ]
    },
    {
        "name": "dragon_egg",
        "info": "龙蛋"
    },
    {
        "name": "dried_kelp_block",
        "info": "干海带块"
    },
    {
        "name": "dripstone_block",
        "info": "滴水石块"
    },
    {
        "name": "dropper",
        "info": "投掷器"
    },
    {
        "name": "emerald_block",
        "info": "绿宝石块"
    },
    {
        "name": "emerald_ore",
        "info": "绿宝石矿石"
    },
    {
        "name": "enchanting_table",
        "info": "附魔台"
    },
    {
        "name": "end_brick_stairs",
        "info": "末地石砖楼梯"
    },
    {
        "name": "end_bricks",
        "info": "末地石砖"
    },
    {
        "name": "end_portal",
        "info": "末地传送门方块"
    },
    {
        "name": "end_portal_frame",
        "info": "末地传送门框架"
    },
    {
        "name": "end_rod",
        "info": "末地烛"
    },
    {
        "name": "end_stone",
        "info": "末地石"
    },
    {
        "name": "ender_chest",
        "info": "末影箱"
    },
    {
        "name": "exposed_copper",
        "info": "斑驳的铜块"
    },
    {
        "name": "exposed_cut_copper",
        "info": "斑驳的切制铜块"
    },
    {
        "name": "exposed_cut_copper_slab",
        "info": "斑驳的切制铜台阶"
    },
    {
        "name": "exposed_cut_copper_stairs",
        "info": "斑驳的切制铜楼梯"
    },
    {
        "name": "exposed_double_cut_copper_slab",
        "info": "斑驳的双层切制铜台阶"
    },
    {
        "name": "farmland",
        "info": "耕地"
    },
    {
        "name": "fence",
        "info": "橡木栅栏/云杉木栅栏/白桦木栅栏/丛林木栅栏/金合欢木栅栏/深色橡木栅栏",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "橡木栅栏"
            },
            {
                "name": "1",
                "info": "云杉木栅栏"
            },
            {
                "name": "2",
                "info": "白桦木栅栏"
            },
            {
                "name": "3",
                "info": "丛林木栅栏"
            },
            {
                "name": "4",
                "info": "金合欢木栅栏"
            },
            {
                "name": "5",
                "info": "深色橡木栅栏"
            }
        ]
    },
    {
        "name": "fence_gate",
        "info": "橡木栅栏门"
    },
    {
        "name": "fire",
        "info": "火"
    },
    {
        "name": "fletching_table",
        "info": "制箭台"
    },
    {
        "name": "flower_pot",
        "info": "花盆"
    },
    {
        "name": "flowering_azalea",
        "info": "盛开的杜鹃花丛"
    },
    {
        "name": "flowing_lava",
        "info": "流动的熔岩"
    },
    {
        "name": "flowing_water",
        "info": "流动的水"
    },
    {
        "name": "frame",
        "info": "物品展示框"
    },
    {
        "name": "frosted_ice",
        "info": "霜冰"
    },
    {
        "name": "furnace",
        "info": "熔炉"
    },
    {
        "name": "gilded_blackstone",
        "info": "镶金黑石"
    },
    {
        "name": "glass",
        "info": "玻璃"
    },
    {
        "name": "glass_pane",
        "info": "玻璃板"
    },
    {
        "name": "glow_frame",
        "info": "荧光物品展示框"
    },
    {
        "name": "glow_lichen",
        "info": "发光地衣"
    },
    {
        "name": "glowstone",
        "info": "荧石"
    },
    {
        "name": "gold_block",
        "info": "金块"
    },
    {
        "name": "gold_ore",
        "info": "金矿石"
    },
    {
        "name": "golden_rail",
        "info": "充能铁轨"
    },
    {
        "name": "granite_stairs",
        "info": "花岗岩楼梯"
    },
    {
        "name": "grass",
        "info": "草"
    },
    {
        "name": "grass_path",
        "info": "草径"
    },
    {
        "name": "gravel",
        "info": "沙砾"
    },
    {
        "name": "gray_candle",
        "info": "灰色蜡烛"
    },
    {
        "name": "gray_candle_cake",
        "info": "插上灰色蜡烛的蛋糕"
    },
    {
        "name": "gray_glazed_terracotta",
        "info": "灰色带釉陶瓦"
    },
    {
        "name": "green_candle",
        "info": "绿色蜡烛"
    },
    {
        "name": "green_candle_cake",
        "info": "插上绿色蜡烛的蛋糕"
    },
    {
        "name": "green_glazed_terracotta",
        "info": "绿色带釉陶瓦"
    },
    {
        "name": "grindstone",
        "info": "砂轮"
    },
    {
        "name": "hanging_roots",
        "info": "垂根"
    },
    {
        "name": "hardened_clay",
        "info": "陶瓦"
    },
    {
        "name": "hay_block",
        "info": "干草块"
    },
    {
        "name": "heavy_weighted_pressure_plate",
        "info": "重质测重压力板"
    },
    {
        "name": "honey_block",
        "info": "蜂蜜块"
    },
    {
        "name": "honeycomb_block",
        "info": "蜜脾块"
    },
    {
        "name": "hopper",
        "info": "漏斗"
    },
    {
        "name": "ice",
        "info": "冰"
    },
    {
        "name": "infested_deepslate",
        "info": "被虫蚀的深板岩"
    },
    {
        "name": "iron_bars",
        "info": "铁栏杆"
    },
    {
        "name": "iron_block",
        "info": "铁块"
    },
    {
        "name": "iron_door",
        "info": "铁门"
    },
    {
        "name": "iron_ore",
        "info": "铁矿石"
    },
    {
        "name": "iron_trapdoor",
        "info": "铁活板门"
    },
    {
        "name": "jukebox",
        "info": "唱片机"
    },
    {
        "name": "jungle_button",
        "info": "丛林木按钮"
    },
    {
        "name": "jungle_door",
        "info": "丛林木门"
    },
    {
        "name": "jungle_fence_gate",
        "info": "丛林木栅栏门"
    },
    {
        "name": "jungle_pressure_plate",
        "info": "丛林木压力板"
    },
    {
        "name": "jungle_stairs",
        "info": "丛林木楼梯"
    },
    {
        "name": "jungle_standing_sign",
        "info": "丛林木告示牌"
    },
    {
        "name": "jungle_trapdoor",
        "info": "丛林木活板门"
    },
    {
        "name": "jungle_wall_sign",
        "info": "墙上的丛林木告示牌"
    },
    {
        "name": "kelp",
        "info": "海带"
    },
    {
        "name": "ladder",
        "info": "梯子"
    },
    {
        "name": "lantern",
        "info": "灯笼"
    },
    {
        "name": "lapis_block",
        "info": "青金石块"
    },
    {
        "name": "lapis_ore",
        "info": "青金石矿石"
    },
    {
        "name": "large_amethyst_bud",
        "info": "大型紫晶芽"
    },
    {
        "name": "lava",
        "info": "熔岩"
    },
    {
        "name": "lava_cauldron",
        "info": "装有熔岩的炼药锅"
    },
    {
        "name": "leaves",
        "info": "橡树树叶/云杉树叶/白桦树叶/丛林树叶",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "橡树树叶"
            },
            {
                "name": "1",
                "info": "云杉树叶"
            },
            {
                "name": "2",
                "info": "白桦树叶"
            },
            {
                "name": "3",
                "info": "丛林树叶"
            }
        ]
    },
    {
        "name": "leaves2",
        "info": "金合欢树叶/深色橡树树叶",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "金合欢树叶"
            },
            {
                "name": "1",
                "info": "深色橡树树叶"
            }
        ]
    },
    {
        "name": "lectern",
        "info": "讲台"
    },
    {
        "name": "lever",
        "info": "拉杆"
    },
    {
        "name": "light_block",
        "info": "光源方块"
    },
    {
        "name": "light_blue_candle",
        "info": "淡蓝色蜡烛"
    },
    {
        "name": "light_blue_candle_cake",
        "info": "插上淡蓝色蜡烛的蛋糕"
    },
    {
        "name": "light_blue_glazed_terracotta",
        "info": "淡蓝色带釉陶瓦"
    },
    {
        "name": "light_gray_candle",
        "info": "淡灰色蜡烛"
    },
    {
        "name": "light_gray_candle_cake",
        "info": "插上淡灰色蜡烛的蛋糕"
    },
    {
        "name": "light_weighted_pressure_plate",
        "info": "轻质测重压力板"
    },
    {
        "name": "lightning_rod",
        "info": "避雷针"
    },
    {
        "name": "lime_candle",
        "info": "黄绿色蜡烛"
    },
    {
        "name": "lime_candle_cake",
        "info": "插上黄绿色蜡烛的蛋糕"
    },
    {
        "name": "lime_glazed_terracotta",
        "info": "黄绿色带釉陶瓦"
    },
    {
        "name": "lit_blast_furnace",
        "info": "燃烧中的高炉"
    },
    {
        "name": "lit_deepslate_redstone_ore",
        "info": "点亮的深层红石矿石"
    },
    {
        "name": "lit_furnace",
        "info": "燃烧中的熔炉"
    },
    {
        "name": "lit_pumpkin",
        "info": "南瓜灯"
    },
    {
        "name": "lit_redstone_lamp",
        "info": "点亮的红石灯"
    },
    {
        "name": "lit_redstone_ore",
        "info": "点亮的红石矿石"
    },
    {
        "name": "lit_smoker",
        "info": "燃烧中的烟熏炉"
    },
    {
        "name": "lodestone",
        "info": "磁石"
    },
    {
        "name": "log",
        "info": "橡木原木/云杉原木/白桦原木/丛林原木",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "橡木原木"
            },
            {
                "name": "1",
                "info": "云杉原木"
            },
            {
                "name": "2",
                "info": "白桦原木"
            },
            {
                "name": "3",
                "info": "丛林原木"
            }
        ]
    },
    {
        "name": "log2",
        "info": "金合欢原木/深色橡木原木",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "金合欢原木"
            },
            {
                "name": "1",
                "info": "深色橡木原木"
            }
        ]
    },
    {
        "name": "loom",
        "info": "织布机"
    },
    {
        "name": "magenta_candle",
        "info": "品红色蜡烛"
    },
    {
        "name": "magenta_candle_cake",
        "info": "插上品红色蜡烛的蛋糕"
    },
    {
        "name": "magenta_glazed_terracotta",
        "info": "品红色带釉陶瓦"
    },
    {
        "name": "magma",
        "info": "岩浆块"
    },
    {
        "name": "medium_amethyst_bud",
        "info": "中型紫晶芽"
    },
    {
        "name": "melon_block",
        "info": "西瓜"
    },
    {
        "name": "melon_stem",
        "info": "西瓜茎"
    },
    {
        "name": "mob_spawner",
        "info": "刷怪笼"
    },
    {
        "name": "monster_egg",
        "info": "被虫蚀的石头/被虫蚀的圆石/被虫蚀的石砖/被虫蚀的苔石砖/被虫蚀的裂纹石砖/被虫蚀的錾制石砖",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "被虫蚀的石头"
            },
            {
                "name": "1",
                "info": "被虫蚀的圆石"
            },
            {
                "name": "2",
                "info": "被虫蚀的石砖"
            },
            {
                "name": "3",
                "info": "被虫蚀的苔石砖"
            },
            {
                "name": "4",
                "info": "被虫蚀的裂纹石砖"
            },
            {
                "name": "5",
                "info": "被虫蚀的錾制石砖"
            }
        ]
    },
    {
        "name": "moss_block",
        "info": "苔藓块"
    },
    {
        "name": "moss_carpet",
        "info": "苔藓地毯"
    },
    {
        "name": "mossy_cobblestone",
        "info": "苔石"
    },
    {
        "name": "mossy_cobblestone_stairs",
        "info": "苔石楼梯"
    },
    {
        "name": "mossy_stone_brick_stairs",
        "info": "苔石砖楼梯"
    },
    {
        "name": "mycelium",
        "info": "菌丝"
    },
    {
        "name": "nether_brick",
        "info": "下界砖"
    },
    {
        "name": "nether_brick_fence",
        "info": "下界砖栅栏"
    },
    {
        "name": "nether_brick_stairs",
        "info": "下界砖楼梯"
    },
    {
        "name": "nether_gold_ore",
        "info": "下界金矿石"
    },
    {
        "name": "nether_sprouts",
        "info": "下界苗"
    },
    {
        "name": "nether_wart",
        "info": "下界疣"
    },
    {
        "name": "nether_wart_block",
        "info": "下界疣块"
    },
    {
        "name": "netherite_block",
        "info": "下界合金块"
    },
    {
        "name": "netherrack",
        "info": "下界岩"
    },
    {
        "name": "normal_stone_stairs",
        "info": "石楼梯"
    },
    {
        "name": "noteblock",
        "info": "音符盒"
    },
    {
        "name": "oak_stairs",
        "info": "橡木楼梯"
    },
    {
        "name": "observer",
        "info": "侦测器"
    },
    {
        "name": "obsidian",
        "info": "黑曜石"
    },
    {
        "name": "orange_candle",
        "info": "橙色蜡烛"
    },
    {
        "name": "orange_candle_cake",
        "info": "插上橙色蜡烛的蛋糕"
    },
    {
        "name": "orange_glazed_terracotta",
        "info": "橙色带釉陶瓦"
    },
    {
        "name": "oxidized_copper",
        "info": "氧化的铜块"
    },
    {
        "name": "oxidized_cut_copper",
        "info": "氧化的切制铜块"
    },
    {
        "name": "oxidized_cut_copper_slab",
        "info": "氧化的切制铜台阶"
    },
    {
        "name": "oxidized_cut_copper_stairs",
        "info": "氧化的切制铜楼梯"
    },
    {
        "name": "oxidized_double_cut_copper_slab",
        "info": "氧化的双层切制铜台阶"
    },
    {
        "name": "packed_ice",
        "info": "浮冰"
    },
    {
        "name": "pink_candle",
        "info": "粉红色蜡烛"
    },
    {
        "name": "pink_candle_cake",
        "info": "插上粉红色蜡烛的蛋糕"
    },
    {
        "name": "pink_glazed_terracotta",
        "info": "粉红色带釉陶瓦"
    },
    {
        "name": "piston",
        "info": "活塞"
    },
    {
        "name": "pistonarmcollision",
        "info": "活塞头"
    },
    {
        "name": "planks",
        "info": "橡木木板/云杉木板/白桦木板/丛林木板/金合欢木板/深色橡木木板",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "橡木木板"
            },
            {
                "name": "1",
                "info": "云杉木板"
            },
            {
                "name": "2",
                "info": "白桦木板"
            },
            {
                "name": "3",
                "info": "丛林木板"
            },
            {
                "name": "4",
                "info": "金合欢木板"
            },
            {
                "name": "5",
                "info": "深色橡木木板"
            }
        ]
    },
    {
        "name": "podzol",
        "info": "灰化土"
    },
    {
        "name": "pointed_dripstone",
        "info": "滴水石锥"
    },
    {
        "name": "polished_andesite_stairs",
        "info": "磨制安山岩楼梯"
    },
    {
        "name": "polished_basalt",
        "info": "磨制玄武岩"
    },
    {
        "name": "polished_blackstone",
        "info": "磨制黑石"
    },
    {
        "name": "polished_blackstone_brick_double_slab",
        "info": "双层磨制黑石砖台阶"
    },
    {
        "name": "polished_blackstone_brick_slab",
        "info": "磨制黑石砖台阶"
    },
    {
        "name": "polished_blackstone_brick_stairs",
        "info": "磨制黑石砖楼梯"
    },
    {
        "name": "polished_blackstone_brick_wall",
        "info": "磨制黑石砖墙"
    },
    {
        "name": "polished_blackstone_bricks",
        "info": "磨制黑石砖"
    },
    {
        "name": "polished_blackstone_button",
        "info": "磨制黑石按钮"
    },
    {
        "name": "polished_blackstone_double_slab",
        "info": "双层磨制黑石台阶"
    },
    {
        "name": "polished_blackstone_pressure_plate",
        "info": "磨制黑石压力板"
    },
    {
        "name": "polished_blackstone_slab",
        "info": "磨制黑石台阶"
    },
    {
        "name": "polished_blackstone_stairs",
        "info": "磨制黑石楼梯"
    },
    {
        "name": "polished_blackstone_wall",
        "info": "磨制黑石墙"
    },
    {
        "name": "polished_deepslate",
        "info": "磨制深板岩"
    },
    {
        "name": "polished_deepslate_double_slab",
        "info": "双层磨制深板岩台阶"
    },
    {
        "name": "polished_deepslate_slab",
        "info": "磨制深板岩台阶"
    },
    {
        "name": "polished_deepslate_stairs",
        "info": "磨制深板岩楼梯"
    },
    {
        "name": "polished_deepslate_wall",
        "info": "磨制深板岩墙"
    },
    {
        "name": "polished_diorite_stairs",
        "info": "磨制闪长岩楼梯"
    },
    {
        "name": "polished_granite_stairs",
        "info": "磨制花岗岩楼梯"
    },
    {
        "name": "portal",
        "info": "下界传送门"
    },
    {
        "name": "potatoes",
        "info": "马铃薯"
    },
    {
        "name": "powder_snow",
        "info": "细雪"
    },
    {
        "name": "powered_comparator",
        "info": "激活的红石比较器"
    },
    {
        "name": "powered_repeater",
        "info": "激活的红石中继器"
    },
    {
        "name": "prismarine",
        "info": "海晶石/暗海晶石/海晶石砖",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "海晶石"
            },
            {
                "name": "1",
                "info": "暗海晶石"
            },
            {
                "name": "2",
                "info": "海晶石砖"
            }
        ]
    },
    {
        "name": "prismarine_bricks_stairs",
        "info": "海晶石砖楼梯"
    },
    {
        "name": "prismarine_stairs",
        "info": "海晶石楼梯"
    },
    {
        "name": "pumpkin",
        "info": "南瓜"
    },
    {
        "name": "pumpkin_stem",
        "info": "南瓜茎"
    },
    {
        "name": "purple_candle",
        "info": "紫色蜡烛"
    },
    {
        "name": "purple_candle_cake",
        "info": "插上紫色蜡烛的蛋糕"
    },
    {
        "name": "purple_glazed_terracotta",
        "info": "紫色带釉陶瓦"
    },
    {
        "name": "purpur_block",
        "info": "紫珀块/錾制紫珀块/紫珀柱/平滑紫珀块",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "紫珀块"
            },
            {
                "name": "1",
                "info": "錾制紫珀块"
            },
            {
                "name": "2",
                "info": "紫珀柱"
            },
            {
                "name": "3",
                "info": "平滑紫珀块"
            }
        ]
    },
    {
        "name": "purpur_stairs",
        "info": "紫珀楼梯"
    },
    {
        "name": "quartz_block",
        "info": "石英块/錾制石英块/石英柱/平滑石英块",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "石英块"
            },
            {
                "name": "1",
                "info": "錾制石英块"
            },
            {
                "name": "2",
                "info": "石英柱"
            },
            {
                "name": "3",
                "info": "平滑石英块"
            }
        ]
    },
    {
        "name": "quartz_bricks",
        "info": "石英砖"
    },
    {
        "name": "quartz_ore",
        "info": "下界石英矿石"
    },
    {
        "name": "quartz_stairs",
        "info": "石英楼梯"
    },
    {
        "name": "rail",
        "info": "铁轨"
    },
    {
        "name": "raw_copper_block",
        "info": "粗铜块"
    },
    {
        "name": "raw_gold_block",
        "info": "粗金块"
    },
    {
        "name": "raw_iron_block",
        "info": "粗铁块"
    },
    {
        "name": "red_candle",
        "info": "红色蜡烛"
    },
    {
        "name": "red_candle_cake",
        "info": "插上红色蜡烛的蛋糕"
    },
    {
        "name": "red_flower",
        "info": "虞美人/兰花/绒球葱/蓝花美耳草/红色郁金香/橙色郁金香/白色郁金香/粉红色郁金香/滨菊/矢车菊/铃兰",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "虞美人"
            },
            {
                "name": "1",
                "info": "兰花"
            },
            {
                "name": "2",
                "info": "绒球葱"
            },
            {
                "name": "3",
                "info": "蓝花美耳草"
            },
            {
                "name": "4",
                "info": "红色郁金香"
            },
            {
                "name": "5",
                "info": "橙色郁金香"
            },
            {
                "name": "6",
                "info": "白色郁金香"
            },
            {
                "name": "7",
                "info": "粉红色郁金香"
            },
            {
                "name": "8",
                "info": "滨菊"
            },
            {
                "name": "9",
                "info": "矢车菊"
            },
            {
                "name": "10",
                "info": "铃兰"
            }
        ]
    },
    {
        "name": "red_glazed_terracotta",
        "info": "红色带釉陶瓦"
    },
    {
        "name": "red_mushroom",
        "info": "红色蘑菇"
    },
    {
        "name": "red_mushroom_block",
        "info": "红色蘑菇方块"
    },
    {
        "name": "red_nether_brick",
        "info": "红色下界砖块"
    },
    {
        "name": "red_nether_brick_stairs",
        "info": "红色下界砖楼梯"
    },
    {
        "name": "red_sandstone",
        "info": "红砂岩/錾制红砂岩/切制红砂岩/平滑红砂岩",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "红砂岩"
            },
            {
                "name": "1",
                "info": "錾制红砂岩"
            },
            {
                "name": "2",
                "info": "切制红砂岩"
            },
            {
                "name": "3",
                "info": "平滑红砂岩"
            }
        ]
    },
    {
        "name": "red_sandstone_stairs",
        "info": "红砂岩楼梯"
    },
    {
        "name": "redstone_block",
        "info": "红石块"
    },
    {
        "name": "redstone_lamp",
        "info": "红石灯"
    },
    {
        "name": "redstone_ore",
        "info": "红石矿石"
    },
    {
        "name": "redstone_torch",
        "info": "红石火把"
    },
    {
        "name": "redstone_wire",
        "info": "红石线"
    },
    {
        "name": "reeds",
        "info": "甘蔗"
    },
    {
        "name": "repeating_command_block",
        "info": "循环型命令方块"
    },
    {
        "name": "respawn_anchor",
        "info": "重生锚"
    },
    {
        "name": "sand",
        "info": "沙子/红沙",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "沙子"
            },
            {
                "name": "1",
                "info": "红沙"
            }
        ]
    },
    {
        "name": "sandstone",
        "info": "砂岩/錾制砂岩/切制砂岩/平滑砂岩",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "砂岩"
            },
            {
                "name": "1",
                "info": "錾制砂岩"
            },
            {
                "name": "2",
                "info": "切制砂岩"
            },
            {
                "name": "3",
                "info": "平滑砂岩"
            }
        ]
    },
    {
        "name": "sandstone_stairs",
        "info": "砂岩楼梯"
    },
    {
        "name": "sapling",
        "info": "橡树树苗/云杉树苗/白桦树苗/丛林树苗/金合欢树苗/深色橡树树苗",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "橡树树苗"
            },
            {
                "name": "1",
                "info": "云杉树苗"
            },
            {
                "name": "2",
                "info": "白桦树苗"
            },
            {
                "name": "3",
                "info": "丛林树苗"
            },
            {
                "name": "4",
                "info": "金合欢树苗"
            },
            {
                "name": "5",
                "info": "深色橡树树苗"
            }
        ]
    },
    {
        "name": "scaffolding",
        "info": "脚手架"
    },
    {
        "name": "sea_pickle",
        "info": "海泡菜"
    },
    {
        "name": "seagrass",
        "info": "海草"
    },
    {
        "name": "sealantern",
        "info": "海晶灯"
    },
    {
        "name": "shroomlight",
        "info": "菌光体"
    },
    {
        "name": "shulker_box",
        "info": "潜影盒"
    },
    {
        "name": "silver_glazed_terracotta",
        "info": "淡灰色带釉陶瓦"
    },
    {
        "name": "skull",
        "info": "生物头颅"
    },
    {
        "name": "slime",
        "info": "黏液块"
    },
    {
        "name": "small_amethyst_bud",
        "info": "小型紫晶芽"
    },
    {
        "name": "small_dripleaf_block",
        "info": "小型垂滴叶"
    },
    {
        "name": "smithing_table",
        "info": "锻造台"
    },
    {
        "name": "smoker",
        "info": "烟熏炉"
    },
    {
        "name": "smooth_basalt",
        "info": "平滑玄武岩"
    },
    {
        "name": "smooth_quartz_stairs",
        "info": "平滑石英楼梯"
    },
    {
        "name": "smooth_red_sandstone_stairs",
        "info": "平滑红砂岩楼梯"
    },
    {
        "name": "smooth_sandstone_stairs",
        "info": "平滑砂岩楼梯"
    },
    {
        "name": "smooth_stone",
        "info": "平滑石头"
    },
    {
        "name": "snow",
        "info": "雪块"
    },
    {
        "name": "snow_layer",
        "info": "雪"
    },
    {
        "name": "soul_campfire",
        "info": "灵魂营火"
    },
    {
        "name": "soul_fire",
        "info": "灵魂火"
    },
    {
        "name": "soul_lantern",
        "info": "灵魂灯笼"
    },
    {
        "name": "soul_sand",
        "info": "灵魂沙"
    },
    {
        "name": "soul_soil",
        "info": "灵魂土"
    },
    {
        "name": "soul_torch",
        "info": "灵魂火把"
    },
    {
        "name": "sponge",
        "info": "海绵/湿海绵",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "海绵"
            },
            {
                "name": "1",
                "info": "湿海绵"
            }
        ]
    },
    {
        "name": "spore_blossom",
        "info": "孢子花"
    },
    {
        "name": "spruce_button",
        "info": "云杉木按钮"
    },
    {
        "name": "spruce_door",
        "info": "云杉木门"
    },
    {
        "name": "spruce_fence_gate",
        "info": "云杉木栅栏门"
    },
    {
        "name": "spruce_pressure_plate",
        "info": "云杉木压力板"
    },
    {
        "name": "spruce_stairs",
        "info": "云杉木楼梯"
    },
    {
        "name": "spruce_standing_sign",
        "info": "云杉木告示牌"
    },
    {
        "name": "spruce_trapdoor",
        "info": "云杉木活板门"
    },
    {
        "name": "spruce_wall_sign",
        "info": "墙上的云杉木告示牌"
    },
    {
        "name": "stained_glass",
        "info": "染色玻璃"
    },
    {
        "name": "stained_glass_pane",
        "info": "染色玻璃板"
    },
    {
        "name": "stained_hardened_clay",
        "info": "染色陶瓦"
    },
    {
        "name": "standing_banner",
        "info": "旗帜"
    },
    {
        "name": "standing_sign",
        "info": "橡木告示牌"
    },
    {
        "name": "sticky_piston",
        "info": "黏性活塞"
    },
    {
        "name": "stickypistonarmcollision",
        "info": "黏性活塞头"
    },
    {
        "name": "stone",
        "info": "石头"
    },
    {
        "name": "stone_brick_stairs",
        "info": "石砖楼梯"
    },
    {
        "name": "stone_button",
        "info": "石质按钮"
    },
    {
        "name": "stone_pressure_plate",
        "info": "石质压力板"
    },
    {
        "name": "stone_slab",
        "info": "平滑石台阶/砂岩台阶/石化橡木台阶/圆石台阶/砖台阶/石砖台阶/石英台阶/下界砖台阶",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "平滑石台阶"
            },
            {
                "name": "1",
                "info": "砂岩台阶"
            },
            {
                "name": "2",
                "info": "石化橡木台阶"
            },
            {
                "name": "3",
                "info": "圆石台阶"
            },
            {
                "name": "4",
                "info": "砖台阶"
            },
            {
                "name": "5",
                "info": "石砖台阶"
            },
            {
                "name": "6",
                "info": "石英台阶"
            },
            {
                "name": "7",
                "info": "下界砖台阶"
            }
        ]
    },
    {
        "name": "stone_slab2",
        "info": "红砂岩台阶/紫珀台阶/海晶石台阶/暗海晶石台阶/海晶石砖台阶/苔石台阶/平滑砂岩台阶/红色下界砖台阶",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "红砂岩台阶"
            },
            {
                "name": "1",
                "info": "紫珀台阶"
            },
            {
                "name": "2",
                "info": "海晶石台阶"
            },
            {
                "name": "3",
                "info": "暗海晶石台阶"
            },
            {
                "name": "4",
                "info": "海晶石砖台阶"
            },
            {
                "name": "5",
                "info": "苔石台阶"
            },
            {
                "name": "6",
                "info": "平滑砂岩台阶"
            },
            {
                "name": "7",
                "info": "红色下界砖台阶"
            }
        ]
    },
    {
        "name": "stone_slab3",
        "info": "末地石砖台阶/平滑红砂岩台阶/磨制安山岩台阶/安山岩台阶/闪长岩台阶/磨制闪长岩台阶/花岗岩台阶/磨制花岗岩台阶",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "末地石砖台阶"
            },
            {
                "name": "1",
                "info": "平滑红砂岩台阶"
            },
            {
                "name": "2",
                "info": "磨制安山岩台阶"
            },
            {
                "name": "3",
                "info": "安山岩台阶"
            },
            {
                "name": "4",
                "info": "闪长岩台阶"
            },
            {
                "name": "5",
                "info": "磨制闪长岩台阶"
            },
            {
                "name": "6",
                "info": "花岗岩台阶"
            },
            {
                "name": "7",
                "info": "磨制花岗岩台阶"
            }
        ]
    },
    {
        "name": "stone_slab4",
        "info": "苔石砖台阶/平滑石英台阶/石台阶/切制砂岩台阶/切制红砂岩台阶",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "苔石砖台阶"
            },
            {
                "name": "1",
                "info": "平滑石英台阶"
            },
            {
                "name": "2",
                "info": "石台阶"
            },
            {
                "name": "3",
                "info": "切制砂岩台阶"
            },
            {
                "name": "4",
                "info": "切制红砂岩台阶"
            }
        ]
    },
    {
        "name": "stone_stairs",
        "info": "石楼梯"
    },
    {
        "name": "stonebrick",
        "info": "石砖/苔石砖/裂纹石砖/錾制石砖",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "石砖"
            },
            {
                "name": "1",
                "info": "苔石砖"
            },
            {
                "name": "2",
                "info": "裂纹石砖"
            },
            {
                "name": "3",
                "info": "錾制石砖"
            }
        ]
    },
    {
        "name": "stonecutter_block",
        "info": "切石机"
    },
    {
        "name": "stripped_acacia_log",
        "info": "去皮金合欢原木"
    },
    {
        "name": "stripped_birch_log",
        "info": "去皮白桦原木"
    },
    {
        "name": "stripped_crimson_hyphae",
        "info": "去皮绯红菌核"
    },
    {
        "name": "stripped_crimson_stem",
        "info": "去皮绯红菌柄"
    },
    {
        "name": "stripped_dark_oak_log",
        "info": "去皮深色橡木原木"
    },
    {
        "name": "stripped_jungle_log",
        "info": "去皮丛林原木"
    },
    {
        "name": "stripped_oak_log",
        "info": "去皮橡木原木"
    },
    {
        "name": "stripped_spruce_log",
        "info": "去皮云杉原木"
    },
    {
        "name": "stripped_warped_hyphae",
        "info": "去皮诡异菌核"
    },
    {
        "name": "stripped_warped_stem",
        "info": "去皮诡异菌柄"
    },
    {
        "name": "structure_block",
        "info": "结构方块"
    },
    {
        "name": "structure_void",
        "info": "结构空位"
    },
    {
        "name": "sweet_berry_bush",
        "info": "甜浆果丛"
    },
    {
        "name": "tallgrass",
        "info": "蕨/草",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "蕨"
            },
            {
                "name": "1",
                "info": "草"
            }
        ]
    },
    {
        "name": "target",
        "info": "标靶"
    },
    {
        "name": "tinted_glass",
        "info": "遮光玻璃"
    },
    {
        "name": "tnt",
        "info": "TNT"
    },
    {
        "name": "torch",
        "info": "火把"
    },
    {
        "name": "trapdoor",
        "info": "活板门"
    },
    {
        "name": "trapped_chest",
        "info": "陷阱箱"
    },
    {
        "name": "tripwire",
        "info": "绊线"
    },
    {
        "name": "tripwire_hook",
        "info": "绊线钩"
    },
    {
        "name": "tuff",
        "info": "凝灰岩"
    },
    {
        "name": "turtle_egg",
        "info": "海龟蛋"
    },
    {
        "name": "twisting_vines",
        "info": "缠怨藤"
    },
    {
        "name": "undyed_shulker_box",
        "info": "潜影盒"
    },
    {
        "name": "unknown",
        "info": "数据更新方块"
    },
    {
        "name": "unlit_redstone_torch",
        "info": "熄灭的红石火把"
    },
    {
        "name": "unpowered_comparator",
        "info": "红石比较器"
    },
    {
        "name": "unpowered_repeater",
        "info": "红石中继器"
    },
    {
        "name": "vine",
        "info": "藤蔓"
    },
    {
        "name": "wall_banner",
        "info": "墙上的旗帜"
    },
    {
        "name": "wall_sign",
        "info": "墙上的橡木告示牌"
    },
    {
        "name": "warped_button",
        "info": "诡异木按钮"
    },
    {
        "name": "warped_door",
        "info": "诡异木门"
    },
    {
        "name": "warped_double_slab",
        "info": "双层诡异木台阶"
    },
    {
        "name": "warped_fence",
        "info": "诡异木栅栏"
    },
    {
        "name": "warped_fence_gate",
        "info": "诡异木栅栏门"
    },
    {
        "name": "warped_fungus",
        "info": "诡异菌"
    },
    {
        "name": "warped_hyphae",
        "info": "诡异菌核"
    },
    {
        "name": "warped_nylium",
        "info": "诡异菌岩"
    },
    {
        "name": "warped_planks",
        "info": "诡异木板"
    },
    {
        "name": "warped_pressure_plate",
        "info": "诡异木压力板"
    },
    {
        "name": "warped_roots",
        "info": "诡异菌索"
    },
    {
        "name": "warped_slab",
        "info": "诡异木台阶"
    },
    {
        "name": "warped_stairs",
        "info": "诡异木楼梯"
    },
    {
        "name": "warped_standing_sign",
        "info": "诡异木告示牌"
    },
    {
        "name": "warped_stem",
        "info": "诡异菌柄"
    },
    {
        "name": "warped_trapdoor",
        "info": "诡异木活板门"
    },
    {
        "name": "warped_wall_sign",
        "info": "墙上的诡异木告示牌"
    },
    {
        "name": "warped_wart_block",
        "info": "诡异疣块"
    },
    {
        "name": "water",
        "info": "水"
    },
    {
        "name": "waterlily",
        "info": "睡莲"
    },
    {
        "name": "waxed_copper",
        "info": "涂蜡铜块"
    },
    {
        "name": "waxed_cut_copper",
        "info": "涂蜡切制铜块"
    },
    {
        "name": "waxed_cut_copper_slab",
        "info": "涂蜡切制铜台阶"
    },
    {
        "name": "waxed_cut_copper_stairs",
        "info": "涂蜡切制铜楼梯"
    },
    {
        "name": "waxed_double_cut_copper_slab",
        "info": "双层涂蜡切制铜台阶"
    },
    {
        "name": "waxed_exposed_copper",
        "info": "斑驳的涂蜡铜块"
    },
    {
        "name": "waxed_exposed_cut_copper",
        "info": "斑驳的涂蜡切制铜块"
    },
    {
        "name": "waxed_exposed_cut_copper_slab",
        "info": "斑驳的涂蜡切制铜台阶"
    },
    {
        "name": "waxed_exposed_cut_copper_stairs",
        "info": "斑驳的涂蜡切制铜楼梯"
    },
    {
        "name": "waxed_exposed_double_cut_copper_slab",
        "info": "斑驳的双层涂蜡切制铜台阶"
    },
    {
        "name": "waxed_oxidized_copper",
        "info": "氧化的涂蜡铜块"
    },
    {
        "name": "waxed_oxidized_cut_copper",
        "info": "氧化的涂蜡切制铜块"
    },
    {
        "name": "waxed_oxidized_cut_copper_slab",
        "info": "氧化的涂蜡切制铜台阶"
    },
    {
        "name": "waxed_oxidized_cut_copper_stairs",
        "info": "氧化的涂蜡切制铜楼梯"
    },
    {
        "name": "waxed_oxidized_double_cut_copper_slab",
        "info": "氧化的双层涂蜡切制铜台阶"
    },
    {
        "name": "waxed_weathered_copper",
        "info": "锈蚀的涂蜡铜块"
    },
    {
        "name": "waxed_weathered_cut_copper",
        "info": "锈蚀的涂蜡切制铜块"
    },
    {
        "name": "waxed_weathered_cut_copper_slab",
        "info": "锈蚀的涂蜡切制铜台阶"
    },
    {
        "name": "waxed_weathered_cut_copper_stairs",
        "info": "锈蚀的涂蜡切制铜楼梯"
    },
    {
        "name": "waxed_weathered_double_cut_copper_slab",
        "info": "锈蚀的双层涂蜡切制铜台阶"
    },
    {
        "name": "weathered_copper",
        "info": "锈蚀的铜块"
    },
    {
        "name": "weathered_cut_copper",
        "info": "锈蚀的切制铜块"
    },
    {
        "name": "weathered_cut_copper_slab",
        "info": "锈蚀的切制铜台阶"
    },
    {
        "name": "weathered_cut_copper_stairs",
        "info": "锈蚀的切制铜楼梯"
    },
    {
        "name": "weathered_double_cut_copper_slab",
        "info": "锈蚀的双层切制铜台阶"
    },
    {
        "name": "web",
        "info": "蜘蛛网"
    },
    {
        "name": "weeping_vines",
        "info": "垂泪藤"
    },
    {
        "name": "wheat",
        "info": "小麦"
    },
    {
        "name": "white_candle",
        "info": "白色蜡烛"
    },
    {
        "name": "white_candle_cake",
        "info": "插上白色蜡烛的蛋糕"
    },
    {
        "name": "white_glazed_terracotta",
        "info": "白色带釉陶瓦"
    },
    {
        "name": "wither_rose",
        "info": "凋零玫瑰"
    },
    {
        "name": "wood",
        "info": "橡木/云杉木/白桦木/丛林木/金合欢木/深色橡木/去皮橡木/去皮云杉木/去皮白桦木/去皮丛林木/去皮金合欢木/去皮深色橡木",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "橡木"
            },
            {
                "name": "1",
                "info": "云杉木"
            },
            {
                "name": "2",
                "info": "白桦木"
            },
            {
                "name": "3",
                "info": "丛林木"
            },
            {
                "name": "4",
                "info": "金合欢木"
            },
            {
                "name": "5",
                "info": "深色橡木"
            },
            {
                "name": "6",
                "info": "去皮橡木"
            },
            {
                "name": "7",
                "info": "去皮云杉木"
            },
            {
                "name": "8",
                "info": "去皮白桦木"
            },
            {
                "name": "9",
                "info": "去皮丛林木"
            },
            {
                "name": "10",
                "info": "去皮金合欢木"
            },
            {
                "name": "11",
                "info": "去皮深色橡木"
            }
        ]
    },
    {
        "name": "wooden_button",
        "info": "橡木按钮"
    },
    {
        "name": "wooden_door",
        "info": "橡木门"
    },
    {
        "name": "wooden_pressure_plate",
        "info": "橡木压力板"
    },
    {
        "name": "wooden_slab",
        "info": "橡木台阶/云杉木台阶/白桦木台阶/丛林木台阶/金合欢木台阶/深色橡木台阶",
        "data": [
            {
                "template": {
                    "input": {
                        "text": "{name} "
                    }
                },
                "__app_list__": true
            },
            {
                "name": "0",
                "info": "橡木台阶"
            },
            {
                "name": "1",
                "info": "云杉木台阶"
            },
            {
                "name": "2",
                "info": "白桦木台阶"
            },
            {
                "name": "3",
                "info": "丛林木台阶"
            },
            {
                "name": "4",
                "info": "金合欢木台阶"
            },
            {
                "name": "5",
                "info": "深色橡木台阶"
            }
        ]
    },
    {
        "name": "wool",
        "info": "羊毛"
    },
    {
        "name": "yellow_candle",
        "info": "黄色蜡烛"
    },
    {
        "name": "yellow_candle_cake",
        "info": "插上黄色蜡烛的蛋糕"
    },
    {
        "name": "yellow_flower",
        "info": "蒲公英"
    },
    {
        "name": "yellow_glazed_terracotta",
        "info": "黄色带釉陶瓦"
    }
]